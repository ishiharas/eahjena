...
export class TimestampService {
    ...
    public _updateAvailable: Observable<{ updatable: boolean, time: Date }>;

    constructor(private http: HttpClient) { 
        this._updateAvailable = this.getLastModifiedData();
    }

    getLastModifiedData(): Observable<{ updatable: boolean, time: Date }> {
        ...
        return this.http
            ...
            .pipe(map(
                (data: CourseData) => {
                    ...
                    if (ls) {
                        return { updatable: (ls == finalDate) ? false : true, time: finalDate};
                    } else {
                        return { updatable: true, time: finalDate};
                    }
                }
            ));
    }
